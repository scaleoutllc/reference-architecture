...
service:
  annotations:
    ...
    external-dns.alpha.kubernetes.io/hostname: "aws-au.fast.dev.wescaleout.cloud,*.aws-au.fast.dev.wescaleout.cloud"
  ports:
  - port: 443
    targetPort: 80
YAML
...
resource "aws_globalaccelerator_endpoint_group" "aws" {
  listener_arn          = data.tfe_outputs.platform-dev-aws-global-load-balancer.values.listener_arn
