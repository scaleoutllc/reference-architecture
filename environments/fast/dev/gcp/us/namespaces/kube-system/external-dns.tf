data "tls_certificate" "gke-oidc" {
  url = data.google_container_cluster.this.self_link
}
...
module "iam-external-dns-for-gke" {
  ...
  version                       = "~> v5.9"
  create_role                   = true
  role_name                     = "external-dns-${data.google_container_cluster.this.name}"
  role_description              = "External DNS role for GKE cluster ${data.google_container_cluster.this.name}"
...
resource "aws_iam_policy" "external-dns" {
  name        = "gke-external-dns-${data.google_container_cluster.this.name}"
  description = "External DNS policy for GKE cluster ${data.google_container_cluster.this.name}"
