...
resource "google_compute_target_https_proxy" "north-south" {
  ...
  certificate_map = "//certificatemanager.googleapis.com/projects/${local.area}/locations/global/certificateMaps/${local.area}"
}
...
resource "aws_route53_record" "north-south" {
  for_each = toset([
    "${local.provider}-${local.locale}.${local.domain}",   // fully-specified apex
    "*.${local.provider}-${local.locale}.${local.domain}", // fully-specified star
  ])
  name    = each.value
