...
service:
  ...
  annotations:
    ...
    cloud.google.com/neg: '{"exposed_ports":{"80":{"name":"${var.name}-north-south"}}}'
    ...
    controller.autoneg.dev/neg: '{"backend_services":{"80":[{"name":"${var.name}-north-south","max_rate_per_endpoint":1000},{"name":"${var.team}-${var.env}-gcp-global-north-south","max_rate_per_endpoint":1000}]}}'
