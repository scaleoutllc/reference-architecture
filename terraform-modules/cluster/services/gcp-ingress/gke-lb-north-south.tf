...
resource "google_compute_target_https_proxy" "north-south" {
  ...
  certificate_map = "//certificatemanager.googleapis.com/projects/${var.project}/locations/global/certificateMaps/${var.cert_map_name}"
}
...
resource "aws_route53_record" "north-south" {
  for_each = toset(var.load_balancer_domains)
  name     = each.value
